...
class Kr36Source(BaseSource):
    ...
    list_url = "https://36kr.com"

    async def parse_list(self, html: str) -> List[Dict[str, Any]]:
        soup = BeautifulSoup(html, "lxml")
        items = []

        for article in soup.select("article, .article-item, .kr-flow-article-item, .article-wrapper"):
            try:
                title_elem = article.select_one("h2 a, h3 a, a.article-item-title, .title a")
                if not title_elem:
                    continue
                ...
                summary_elem = article.select_one(".article-summary, .summary, .article-desc, p.desc")
                summary = summary_elem.get_text(strip=True) if summary_elem else ""

                author_elem = article.select_one(".author, .author-name, .article-author, .name")
                author = author_elem.get_text(strip=True) if author_elem else ""

                time_elem = article.select_one(".time, .date, .publish-time, .kr-time")
                publish_time = time_elem.get_text(strip=True) if time_elem else ""

                tags = []
                for tag_elem in article.select(".tag, .column, .article-tags a, .category"):
                    tag_text = tag_elem.get_text(strip=True)
                    if tag_text:
                        tags.append(tag_text)

                heat_elem = article.select_one(".view-count, .read-count, .play-count")
                heat = None
                if heat_elem:
    ...
    async def parse_detail(self, html: str, article_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            time_elem = soup.select_one(".time, .publish-time, .article-time")
            if time_elem and not article_data.get("publish_time"):
